...
<div class="push-top">
  ...
  <table class="table">
    ...
    <tbody>
        ...
        <tr>
            <td>{{$tuteur->id}}</td>
            <td><a href="{{ route('tuteurs.show', $tuteur->id)}}">{{$tuteur->user->name}}</a></td>
            <td>{{$tuteur->user->prenom}}</a></td>
            <td>{{$tuteur->user->adresse}}</a></td>
            <td>{{$tuteur->user->tele}}</a></td>
            <td>{{$tuteur->user->email}}</a></td>
            <td>{{$tuteur->created_at->diffForHumans()}}</td>
            <td class="text-center">
                <a href="{{ route('tuteurs.edit', $tuteur->id)}}" class="btn btn-primary btn-sm"">Edit</a>
                <form action="{{ route('tuteurs.destroy', $tuteur->id)}}" method="post" style="display: inline-block">
                    @csrf
                    @method('DELETE')
