...
@section('title', 'Liste Associations')
  ...
  <table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><a href="{{ route('associations.show', $association->id)}}">{{$association->titre}}</a></td>
            <td><a href="{{ route('communes.show', $association->commune->id)}}">{{$association->commune->titre}}</a></td>
            <td>{{  $association->created_at->diffForHumans() }}</td>
            <td class="text-center">
                <a href="{{ route('associations.edit', $association->id)}}" class="btn btn-primary btn-sm"">Edit</a>
                <form action="{{ route('associations.destroy', $association->id)}}" method="post" style="display: inline-block">
                    @csrf
                    @method('DELETE')
